                        ...
                        <div class="col-lg-12 page-posts">
                            <div class="row">
                                @if (!is_null($postsOfType) && count($postsOfType) > 0)
                                    ...
                                    <div class="accordion" id="accordionExample alert alert-light border border-primary">
                                        <div class="accordion-item">
                                          <h2 class="accordion-header" id="{{ 'headingOne'.$postOfType->id }}">
                                            <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="{{ '#collapseOne'.$postOfType->id }}" aria-expanded="true" aria-controls="{{ 'collapseOne'.$postOfType->id }}">
                                                {{ $postOfType->post_title }}
                                            </button>
                                          </h2>
                                          <div id="{{ 'collapseOne'.$postOfType->id }}" class="accordion-collapse collapse {{ ($loop->first) ? 'show' : '' }}" aria-labelledby="{{ 'headingOne'.$postOfType->id }}" data-bs-parent="#accordionExample">
                                            <div class="accordion-body">
                                                @if ($postOfType->post_featured_image)
                                                <div class="set-bg rounded post-featured-image" data-setbg="{{ asset($postOfType->post_featured_image) }}"></div>
                                                @endif
                                                <div class="py-3">
                                                    {!! $postOfType->post_content !!}
                                                </div>
                                            </div>
                                          </div>
                                        </div>
